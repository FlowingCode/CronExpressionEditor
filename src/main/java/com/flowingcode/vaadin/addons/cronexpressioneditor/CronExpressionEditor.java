...
public class CronExpressionEditor extends CustomField<String> {
  ...
  private String[] cronExpressionSplit;
  ...
  private void configureLayout() {
    ...
    inputExpressionTf.addValueChangeListener(v -> {
      ...
      cronExpressionSplit = inputExpressionTf.getValue().split(" ");
  ...
  private void setDailyLayout() {
    ...
    ComboBox<String> timeUnitCb = new ComboBox<>();
    timeUnitCb.setClassName("inline-combobox");
    timeUnitCb.setWidth("50%");
    timeUnitCb.setItems(i18n.getUnits());
    timeUnitCb.setValue(i18n.getUnits().get(0));
    timeUnitCb.addValueChangeListener(v -> {
      if (v.getValue().equals("Hours")) {
  ...
  private void getDailyExpression(String timeUnit, int value) {
    Map<Integer, String> values = new HashMap<>();
  ...
  private void showNextDates() {
    if (!inputExpressionTf.getValue().isEmpty()) {
      ...
      grid.setHeight("30%");
      grid.setItems(DataProvider.fromCallbacks(q -> generateNextDate(LocalDateTime.now(), q.getOffset(), q.getLimit()),
          q -> Integer.MAX_VALUE));
      add(grid);
    }
  }
  ...
  /**
   * Displays a button that generates a grid with the next dates scheduled by the cron expression generated
   * 
   ...
   * user to chose from one of them.
   * 
